...
+ (instancetype)showOnView:(UIView *)view andString:(NSString *)string{
    
    WSProgressHUD *hud = [[WSProgressHUD alloc] initWithView:view];
    [view addSubview:hud];
    [hud showWithString:string maskType:WSProgressHUDMaskTypeDefault];
    return hud;
}

- (void)showString:(NSString *)string dimissAfterSecond:(NSInteger)second{
    [self showImage:nil status:string maskType:WSProgressHUDMaskTypeDefault];
